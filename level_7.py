def run_level_7(screen, hint_manager, preserve_state=False):
    import pygame
    import time
    import os
    import settings
    ...
    if not hasattr(run_level_7, 'base_start_time'):
        run_level_7.base_start_time = None
        run_level_7.paused_time = 0
        run_level_7.timer_paused = False
        run_level_7.pause_start = 0

    if preserve_state and run_level_7.base_start_time is not None:
        start_timer = run_level_7.base_start_time
        paused_time = run_level_7.paused_time
        timer_paused = run_level_7.timer_paused
        pause_start = run_level_7.pause_start
    else:
        start_timer = pygame.time.get_ticks()
        run_level_7.base_start_time = start_timer
        paused_time = 0
        run_level_7.paused_time = 0
        timer_paused = False
        run_level_7.timer_paused = False
    ...
    if not preserve_state:
        ...
        if settings.music_on:
            pygame.mixer.music.load(os.path.join("materials", "bgm", "lv7.mp3"))
    ...
    while not level_complete:
        ...
        for i in range(1, 2):
            ...
            puzzles.append({"img": paper, "rect": rect_paper, "original_rect": rect_paper.copy()})
        ...
        while run:
            ...
            if settings_open and not timer_paused:
                ...
                run_level_7.timer_paused = True
                run_level_7.pause_start = timer_pause_start
            elif not settings_open and timer_paused:
                timer_paused = False
                paused_time += pygame.time.get_ticks() - timer_pause_start
                run_level_7.paused_time = paused_time
                run_level_7.timer_paused = False
                ...
                if event.type == pygame.QUIT:
                    ...
                    if not settings_open:
                        if active_puzzle is not None:
                            ...
                            puzzles[active_paper]["rect"].move_ip(event.rel)
                            paper_rect = puzzles[active_paper]["rect"]
